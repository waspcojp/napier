...
ul a.file.gif::before,ul a.file.jpg::before,ul a.file.png::before,ul a.file.svg::before {
	...
	<body>
		<main>
            ...
            <ul id="files">
                {#each files as value}
					<li>
                        <a href={value.base} title={value.title} class='{value.type} {value.ext}'>
						    {value.base}
						</a>
                    </li>
				{/each}
...
let {paths, files, directory} = procs;

beforeUpdate(() => {
	console.log('index beforeUpdate', procs);
});
